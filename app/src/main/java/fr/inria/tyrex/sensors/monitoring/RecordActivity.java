...
public class RecordActivity extends Activity {
	...
	private void save() {
		...
		File directoryFile = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + directory);
		if (!directoryFile.exists()) {
			if(!directoryFile.mkdir()) {
				Toast.makeText(getApplicationContext(), "Impossible to create " + directoryFile.getAbsolutePath(), Toast.LENGTH_LONG).show();
			}
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (!new File(mZipFile).delete()) {
			...
			Toast.makeText(getApplicationContext(), "Impossible to delete " + mZipFile, Toast.LENGTH_LONG).show();
		}
	}
